...
@testset "PlugFlowReactor.jl" begin
    ...
    @testset "Testing gas chemistry and surface chemistry together" begin
        input_file = joinpath("plug_gas_and_surf", "plug.xml")
        retcode = plug(input_file, lib_dir, gaschem=true, surfchem=true)
        @test retcode == Symbol("Success")
    end
